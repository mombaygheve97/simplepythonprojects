...
root = Tk() 
root.title('GMNotifier Alarm-Clock') 
...
def alarmclock(alarmtime): 
    while True:
        ...
        if time_now==alarmtime:
            ...
            text="Time to love Nar",bg="DodgerBlue2",fg="white").grid(row=6,columnspan=3)
            print("wake up!")
            mixer.init()
            #mixer.music.load(r'C:\Files\Gheve\Downloads\B2alarm.mp3')
            #mixer.music.play()
            break
...
text="Alarm").grid(row=1,columnspan=3)
...
setbtn2=Button(root,text="close",command=endalarm,bg="DodgerBlue2",
